...
object Events {
  ...
  def getKeyboard(eventIndex: Int): InlineKeyboardMarkup =
    ...
    else if (eventIndex == Events.events.length - 1)
      InlineKeyboardMarkup(List(List(InlineKeyboardButton("prev", callbackData = (eventIndex - 1).toString))))
    else
      InlineKeyboardMarkup(
        List(
          List(InlineKeyboardButton("prev", callbackData = (eventIndex - 1).toString),
...
object EventsScraper {
  ...
  // Unused now
  lazy val menu =
    browser.get(baseUrl + "?category=2") >> elementList(
      "div.toolbar-group:nth-child(2) > " +
        "div:nth-child(1) > " +
        "ul:nth-child(2) > " +
        "li > a")
  // Unused now
  ...
  def parseEvent(eventDiv: Element): Event = {
    ...
    val endDate = DateTime
      .parse(dateTimeElem >> attr("content")("meta:last-of-type"))
      .withHourOfDay(endHour)
      .withMinuteOfHour(endMin) // the end/start date are used to hold the times, see Event#isValidToShow()
    // Location
                 ...
                 eventDiv: Element) // just in case
{
  ...
  override def toString() = {
    ...
    //.appendSeparator(" and ")
      ...
      .toFormatter()
    val dateFmter = new DateTimeFormatterBuilder()
      .appendDayOfWeekText()
      .appendLiteral(", ")
