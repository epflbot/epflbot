...
trait Menus extends Commands with Callbacks { _: TelegramBot =>
  ...
  on("/menus", "today's menus") { implicit msg => args =>
    val restos = MenusScraper.retrieveMenus().groupBy(_.resto).filterKeys {
      ...
      key =>
        if (args.isEmpty) true else buildPredicate(key, args)
    } map {
    ...
    InlineKeyboardMarkup(
      List(
        (
          ...
          else if (restoIndex == cached.get.length - 1)
            List(InlineKeyboardButton("prev", callbackData = callbackPrefix + (restoIndex - 1)))
          else
            List(InlineKeyboardButton("prev", callbackData = callbackPrefix + (restoIndex - 1)),
...
case class Menu(description: String, resto: String, prix: List[(String, String)], eventDiv: Element) // just in case
{
